...
const engines: Engine[] = ['tesseract', 'paddle'];
export default function OcrLab() {
    ...
    try {
      for (const sample of snapshot) {
        ...
        for (const engine of engines) {
          ...
          setBusy(`${engine === 'paddle' ? 'PaddleOCR' : 'Tesseract'} · ${sample.file.name}`);
  ...
  function exportReport() {
    const report = { version:1, createdAt:new Date().toISOString(), preprocessing:'original colour image; no thresholding or masking', paddleModel:'PP-OCRv5_mobile_det + PP-OCRv5_mobile_rec', tesseractLanguage:'eng', normalization:'NFC; whitespace collapsed; case and punctuation preserved', memory:'not measured', samples:samples.map(s=>({name:s.file.name,kind:s.kind,bytes:s.file.size,reference:s.reference,results:Object.fromEntries(engines.map(e=>{const r=s.results[e];return [e,r && !('error' in r)?{...r,errors:errorRates(s.reference,r.text)}:r??null];}))})) };
  ...
  return <main className="max-w-6xl mx-auto p-5 sm:p-8">
    ...
    <p className="mt-3 text-stone-600 max-w-3xl">Compare original screenshots with Tesseract and PaddleOCR. Add the correct transcription to measure recognition errors. This experiment does not change your library or its default OCR engine.</p>
    <div className="rounded-xl border border-brand-100 bg-white p-4 my-5 text-sm space-y-2"><p>Models download on first use; recognition runs in browser workers. Each engine runs separately. Setup, recognition and total elapsed time are reported separately; memory is not measured.</p><p>Include printed text, highlighted notes, handwriting and calligraphy. These labels describe your samples; annotation understanding is not implemented here. English Tesseract and multilingual PaddleOCR use different models, so this compares our proposed configurations.</p><p>Optional PaddleOCR setup: <code>pnpm setup:paddle</code>, then restart/rebuild. Its integration is experimental and has not yet been runtime-verified.</p></div>
    ...
    }}/></label><button className="small-primary text-sm" disabled={!samples.length||!!busy} onClick={()=>void run()}>Compare both engines</button>{busy&&<button className="text-sm underline" onClick={()=>control.current?.abort()}>Cancel</button>}<button className="inline-flex gap-2 text-sm text-brand-700" disabled={!!busy||!samples.some(s=>Object.keys(s.results).length)} onClick={exportReport}><Download size={16}/>Export report</button></div>
